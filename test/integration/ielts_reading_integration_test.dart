...
void main() {
  ...
  test('All combos parse with valid article and questions', () async {
    ...
    for (final c in combos) {
      ...
      if (r.title == '?') {
        unknownTitles++;
        titleFailures.add(TitleFailure(
          comboId: c.id,
          seriesId: c.seriesId,
          testId: c.testId,
          part: c.part,
      ...
      final validQs = r.questions.where((q) => q.displayText.isNotEmpty).toList();
      ...
      if (exp != null && exp > 0 && validQs.length != exp) {
        countMismatches++;
        countMismatchFailures.add(CountMismatch(
          comboId: c.id,
          seriesId: c.seriesId,
          testId: c.testId,
          part: c.part,
    ...
    await _writeFailureReport(titleFailures, countMismatchFailures, <AnswerMismatch>[], <SkippedTypeFailure>[]);
  });
    ...
    for (final c in combos) {
      ...
      for (int i = 0; i < r.questions.length; i++) {
        ...
        if (q.type == 'select-section-given-list' || q.type == 'select-section') {
          skippedTypesCount++;
          skippedTypes.add(SkippedTypeFailure(
            comboId: c.id,
            seriesId: c.seriesId,
            testId: c.testId,
            part: c.part,
            ...
            questionIndex: i,
            type: q.type,
            reason: 'Roman numeral answers in textLookup but parser uses optionLookup',
            options: q.options,
          ));
          continue;
        }
        ...
        if (!_matchesAnswer(q, qId, ans)) {
          ...
          mismatches.add('${c.id} q$qId (type:${q.type}): expected $expected, got $actual');
          answerMismatches.add(AnswerMismatch(
            comboId: c.id,
            seriesId: c.seriesId,
            testId: c.testId,
            part: c.part,
  ...
  test('ReadingNoti loads passage end-to-end', () async {
    ...
    for (final c in combos) {
      ...
      expect(notifier.hasError, isFalse, reason: 'Notifier error ${c.id}: ${notifier.errorMessage}');
...
bool _matchesAnswer(ParagraphGroup q, int qId, List answers) {
  ...
  if (q.type.startsWith('select-') || q.type == 'checkbox' || q.type == 'option-abc') {
    final options = q.options;
    if (options.isEmpty) {
      if (q.type == 'select-section-given-list' || q.type == 'select-section') {
        final romanMap = {
          'I': 0, 'II': 1, 'III': 2, 'IV': 3, 'V': 4, 'VI': 5, 'VII': 6, 
          'VIII': 7, 'IX': 8, 'X': 9, 'XI': 10, 'XII': 11, 'XIII': 12,
        };
        final expectedNorm = expectedRaw.toUpperCase().trim();
        if (romanMap.containsKey(expectedNorm)) {
          return q.answers.isNotEmpty && q.answers.first == romanMap[expectedNorm]!;
        }
      }
      return true;
    }
